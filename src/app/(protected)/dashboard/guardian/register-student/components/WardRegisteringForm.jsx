...
function WardRegisteringForm({ guardianId = 0}) {
  ...
  const didGuard = useRef(false);
  useEffect(() => {
    if (didGuard.current) return;
    didGuard.current = true;

    if (!guardianId && session) {
  ...
  return (
    <div className="mt-3 bg-gray-200 dark:bg-gray-800 p-3 rounded-2xl md:w-150">
      <p className={`${state.success ? "text-green-500" : "text-red-600"} p-3`}>
        {state.message}
        {isPending && <span>Registrando...</span>}
      </p>
      <form action={action}>
        {!!guardianId && (
          <input
...
export default WardRegisteringForm;
